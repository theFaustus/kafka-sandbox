...
@Slf4j
public class KafkaProducerTransactionJson {
        ...
        try (KafkaProducer<String, BankTransaction> kafkaProducer = new KafkaProducer<>(kafkaProps)) {
            while (j <= 10) {
                ...
                while (i <= 100) {
                    ...
                    kafkaProducer.send(new ProducerRecord<>(BankUserJoinApp.BANK_ACCOUNT_INPUT_TOPIC, tx.name(), tx), (metadata, exception) -> log.info("Received response {}", metadata));
                    i++;
                }
                Thread.sleep(1000);
                j++;
